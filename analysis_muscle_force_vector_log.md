...
### 2026-07-25 addendum -- root cause found: `make_stimuli()` metadata/
### config-consistency bug (Python rig code). FOLLOW-UP TODO, NOT FIXED HERE.

**Question asked:** was the L/R swap above a RIG COMMAND issue (wrong
physical side actually stimulated) or a METADATA issue (right side
stimulated, wrong label recorded)? Quick trace of the Python export/rig
code, `bender_h5_export.py` + `bender_functions.py`:

- `stim_side` is written by `bender_h5_export.py` from `s1`/`s2` (the two
  AO stim channels' activity), hardcoding `s1 = "left"`, `s2 = "right"`.
  `_deposit_stim_on_side` correctly routes to `s1`/`s2` per the
  `S1side`/`S2side` config -- this half is self-consistent, not the bug.
- `make_stimuli()` (`bender_functions.py`) builds the commanded waveform
  as `angle = amp * sin(2*pi*tnorm)` and anchors the "Left-phase burst" at
  `tnorm = 0.25` -- i.e. it HARDCODES that "left" = the POSITIVE `sin`
  peak (the `positive_motor_direction`-side shortening extremum),
  regardless of what `positive_motor_direction` is actually configured
  to for a given rig setup. It never reads/branches on
  `positive_motor_direction` when deciding which extremum gets the
  "left" burst.
- For bass16/17/18's specific config, `positive_motor_direction` is set
  such that this hardcoded assumption is backward relative to the
  PI-confirmed intended design (phase=0 = stim centered on the RECRUITED
  muscle's own peak STRETCH, not its own peak shortening) -- which is
  exactly the mismatch found empirically above.

**CONCLUSION: this is a metadata/config-consistency bug, not a
mis-wiring or wrong-channel command bug.** The rig stimulated a
self-consistent side per its own internal wiring logic; `make_stimuli()`
just mislabels which physical extremum that side's burst is centered on
whenever `positive_motor_direction` differs from the value it silently
assumes. Command amplitude/timing/duty/frequency are NOT suspect.

**FOLLOW-UP TODO (not fixed in this pass -- flagging per user request to
revisit later):**
1. In `bender_functions.py::make_stimuli()`, make the burst-to-extremum
   assignment reference `positive_motor_direction` explicitly instead of
   hardcoding the `sin`/`tnorm=0.25` peak as "left." Confirm against
   `S1side`/`S2side` + `positive_motor_direction` for at least one rig
   config where they're known to disagree (this bass16/17/18 config) and
   one where they might already agree, before treating any fix as done.
2. This is a Python/rig-code change -- out of scope for "propose one fix
   at a time" in an R-analysis session; needs its own dedicated pass with
   hardware-affecting-edit safety checklist per `.cursorrules`
   ("Never run hardware-affecting code without explicit user
   confirmation").
3. Existing already-recorded HDF5 files do NOT need re-export/re-
   derivation once `make_stimuli()` is fixed -- the R-side `rec_lidx`
   swap (this file, previous addendum) already compensates for the
   existing files' mislabeling at the point of use. A future
   `make_stimuli()` fix only affects NEWLY-recorded files; if/when that
   lands, the R-side swap will need to be made CONDITIONAL on an
   acquisition-date/rig-software-version cutoff rather than applied
   unconditionally, or the newly-recorded files will get double-corrected
   (swapped back to backward). Flag this explicitly at fix time.

### 2026-07-25 addendum -- canonical dynamic-force plots regenerated
### after the L/R swap fix (bc29776)

Per the SCOPE note above, only figures depending on `td$muscle_force_Nm`
for dynamic trials (the per-sample, side-corrected display/tension
channel) were stale. Whole-cycle `avg_power.W`/`work.J` numbers are
UNCHANGED by this fix (raw-sign, independent of `rec_lidx`) and are NOT
regenerated here -- they remain pending the separate contraction-phase-
classification decision (2026-07-25 addendum above, "SCOPE of what
changed").

Regenerated by re-running `R/run_fv_fl_power_pipeline.R` for all three
vector-force specimens (`BENDER3_SOURCE_DIR`/`BENDER3_OUTPUT_DIR`
overrides per `paths_config.R`'s `BASS{16,17,18}_RAW_SUBFOLDER`/
`figs_dir()`), which now carries the bc29776 fix. This regenerates
every dynamic per-trial compound plot (`{tid}.png`, `muscle_force_Nm`
panel) AND the pooled dynamic force-vs-time summary plots, in each
specimen's `figs_{specimen}/` folder (`02_processed/figs_bass{16,17,18}/`
under `proj_crittergripper`):
- `{tid}.png` for every dynamic trial (compound plot's force panel)
- `forceTime_dynamic_legacy.png`
- `forceTime_dynamic_by_frequency.png`
- `forceTime_dynamic_by_amplitude.png`
- `forceTime_dynamic_by_phase.png`
- `forceTime_dynamic_by_duty.png`
