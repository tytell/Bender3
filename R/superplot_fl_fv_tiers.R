...
p1 <- ggplot(wt_fl, aes(shortening_strain_pct, force_geom_N,
  ...
  geom_line(linewidth = 0.4, alpha = 0.85) + geom_point(size = 0.8, alpha = 0.7) +
  ...
  labs(title = "Within-trial FL (isometric) -- each line = ONE trial's own length series, geometric u_hat, RAW force (no cross-trial normalization)",
       subtitle = "Cleanest FL view: same session/F0/passive. Free y per fish (bass18 is much stronger). Spread across lines = between-trial variability, not shape.",
...
p3 <- ggplot() +
  ...
  geom_point(data = wp_fl, aes(shortening_strain_pct, force_geom_norm, color = fish), size = 0.9, alpha = 0.35) +
  ...
  labs(title = "Within-protocol pooled FL -- ISOMETRIC ONLY, across fish, F/F0 (geometric u_hat)",
       subtitle = "Companion to the across-protocol FLsuperplot but with NO isovelocity-V=0 / dynamic-L0 mixing -- the isometric FL shape under its own relaxation-fit passive. Black = grand mean.",
